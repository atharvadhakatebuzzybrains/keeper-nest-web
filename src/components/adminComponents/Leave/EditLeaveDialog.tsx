...
interface LeaveType {
  ...
  isActive: boolean; // New field to track activation status
...
export default function EditLeaveDialog({ isOpen, currentLeave, onClose, onSubmit }: EditLeaveDialogProps) {
  const [formData, setFormData] = useState({
    leaveName: currentLeave.name || '',
    shortCode: currentLeave.shortCode || '',
    color: currentLeave.color || 'blue',
    creditType: (currentLeave.creditType || 'yearly') as 'yearly' | 'monthly-accrual' | 'not-applicable',
    totalDaysYear: currentLeave.totalDaysYear || '',
    creditDate: currentLeave.creditDate || 'january-1st',
    customCreditDate: currentLeave.customCreditDate || '',
    description: currentLeave.description || '',
  });

  const [creditRules, setCreditRules] = useState<ToggleItem[]>(
    currentLeave.creditRules && currentLeave.creditRules.length > 0
      ? currentLeave.creditRules
      : [
  ...
  const [yearEndRules, setYearEndRules] = useState<ToggleItem[]>(
    currentLeave.yearEndRules && currentLeave.yearEndRules.length > 0
      ? currentLeave.yearEndRules
      : [
  ...
  const [consumptionRules, setConsumptionRules] = useState<ToggleItem[]>(
    currentLeave.consumptionRules && currentLeave.consumptionRules.length > 0
      ? currentLeave.consumptionRules
      : [
  ...
  // Update form data when currentLeave changes
  useEffect(() => {
    if (isOpen && currentLeave.id) {
      setFormData({
        leaveName: currentLeave.name || '',
        shortCode: currentLeave.shortCode || '',
        color: currentLeave.color || 'blue',
      ...
      if (currentLeave.creditRules && currentLeave.creditRules.length > 0) {
        setCreditRules(currentLeave.creditRules);
      }

      if (currentLeave.yearEndRules && currentLeave.yearEndRules.length > 0) {
        setYearEndRules(currentLeave.yearEndRules);
      }

      if (currentLeave.consumptionRules && currentLeave.consumptionRules.length > 0) {
        setConsumptionRules(currentLeave.consumptionRules);
      }
    }
  }, [isOpen, currentLeave]);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
        <DialogHeader className="space-y-2">
          <DialogTitle className="text-2xl">Edit Leave Type</DialogTitle>
          <DialogDescription>
            Update leave type configuration with credit rules, consumption rules, and special policies
          </DialogDescription>
        </DialogHeader>
